...
class YPort : SpeedMode("YPort") {
    ...
    override fun onMotion() {
        if (!safeJump && !mc.gameSettings.keyBindJump.isKeyDown && !(mc.thePlayer ?: return).isOnLadder && !(mc.thePlayer
                ?: return).isInsideOfMaterial(
                Material.water
            ) && !(mc.thePlayer ?: return).isInsideOfMaterial(Material.lava) && !(mc.thePlayer ?: return).isInWater && (this.getBlock(-1.1) != Blocks.air && this.getBlock(
                -1.1
            ) != Blocks.air || this.getBlock(-0.1) != Blocks.air && (mc.thePlayer ?: return).motionX != 0.0 && (mc.thePlayer
                ?: return).motionZ != 0.0 && !(mc.thePlayer ?: return).onGround && (mc.thePlayer
                ?: return).fallDistance < 3.0f && (mc.thePlayer ?: return).fallDistance > 0.05) && level == 3
        ) (mc.thePlayer ?: return).motionY = -0.3994
        ...
        if (!MovementUtils.isMoving) safeJump = true else if ((mc.thePlayer ?: return).onGround) safeJump = false
    }
    ...
    override fun onMove(event: MoveEvent) {
        ...
        if ((mc.thePlayer ?: return).onGround && MovementUtils.hasMotion()) level = 2
        if (round((mc.thePlayer ?: return).posY - (mc.thePlayer ?: return).posY.toInt()) == round(0.138)) {
        ...
        if (level == 1 && ((mc.thePlayer ?: return).moveForward != 0.0f || (mc.thePlayer ?: return).moveStrafing != 0.0f)) {
            ...
            if ((mc.theWorld ?: return).getCollidingBoundingBoxes(
                ...
                ).size > 0 || (mc.thePlayer ?: return).isCollidedVertically
            ) level = 1
            moveSpeed = lastDist - lastDist / 159.0
        }
        moveSpeed = moveSpeed.coerceAtLeast(baseMoveSpeed)
        var forward: Float = (mc.thePlayer ?: return).movementInput.moveForward
        var strafe: Float = (mc.thePlayer ?: return).movementInput.moveStrafe
    ...
    private fun getBlock(axisAlignedBB: AxisAlignedBB): Block? {
        for (x in floor(axisAlignedBB.minX).toInt() until floor(axisAlignedBB.maxX).toInt() + 1) {
            for (z in floor(axisAlignedBB.minZ).toInt() until floor(axisAlignedBB.maxZ).toInt() + 1) {
                val block = (mc.theWorld ?: return null).getBlockState(BlockPos(x, axisAlignedBB.minY.toInt(), z)).block
