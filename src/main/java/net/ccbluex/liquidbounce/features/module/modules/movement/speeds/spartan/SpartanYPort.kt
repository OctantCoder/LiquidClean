    ...
    override fun onMotion() {
        if (mc.gameSettings.keyBindForward.isKeyDown && !mc.gameSettings.keyBindJump.isKeyDown) {
            if ((mc.thePlayer ?: return).onGround) {
                ...
                if (airMoves >= 3) (mc.thePlayer ?: return).jumpMovementFactor = 0.0275f
                if (airMoves >= 4 && airMoves % 2.toDouble() == 0.0) {
                    (mc.thePlayer ?: return).motionY = -0.32f - 0.009 * Math.random()
                    (mc.thePlayer ?: return).jumpMovementFactor = 0.0238f
                }
                airMoves++
            }
        }
